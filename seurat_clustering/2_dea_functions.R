...
GeneMarkersTable <- function(
  ...
  cells.1 <- WhichCells(object = seurat_object, idents = ident.1)
  exp1 <- apply(
    GetAssayData(object = seurat_object, assay = "RNA", layer = "merged.data")[, cells.1, drop = F], 
    1, 
    function(x) mean(x = expm1(x = x)) # reverse log-normalization 
  )
  cells.2 <- WhichCells(object = seurat_object, idents = ident.2)
  exp2 <- apply(
    GetAssayData(object = seurat_object, assay = "RNA", layer = "merged.data")[, cells.2, drop = F],
    1, 
    function(x) mean(x = expm1(x = x))
  )
  
  # Determine DE between the two groups
  DefaultLayer(seurat_object[["RNA"]]) <- "merged.data"
...
ClusterGeneMarkersByPairs <- function(
  ...
  if (id != "") { Idents(seurat_object) <- id }
  # If clusters is not provided, use all cluster levels in the active identity
  if (length(clusters) == 0) { clusters <- levels(seurat_object@active.ident) }
...
FilterClusterGeneMarkersPairs <- function(
  ...
  if (id != "") Idents(seurat_object) <- id
  if (length(clusters) == 0) clusters <- levels(Idents(seurat_object))
  ...
  for (i in 1:(length(clusters)-1)) {
    for (j in (i+1):length(clusters)) {
      ...
      if (file.exists(deg.table)) {
        ...
        if (heatmap) {
          cells <- colnames(
            GetAssayData(seurat_object, layer = "merged.data"))[Idents(seurat_object) %in% sort(
              c(clusters[i], clusters[j])
            )]
          plot.name <- paste0(
            out.dir, "/heatmap/heatmap_DEG_", test.use, "_cl", clusters[i], "-", clusters[j], ".png"
          )
          HeatmapPlot(seurat_object, filt.table, plot.name, cells = cells)
        }
        if (volcano) {
          volcano.plot <- paste0(
...
FilterClusterGeneMarkersAll <- function(
  ...
  cells <- colnames(GetAssayData(seurat_object, layer = "merged.data"))[Idents(seurat_object) %in% clusters]
