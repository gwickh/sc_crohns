...
REDUCT_NAME = "X_embeddings_" + label
...
plot = sc.pl.umap(
    adata, color=label, ncols=2, frameon=True, return_fig=True, show=False
)
...
plot = sc.pl.umap(
    adata, color="sample_id", ncols=4, frameon=True, return_fig=True, show=False
)
plot.set_size_inches(6, 6)
plot.savefig(
...
plot = sc.pl.umap(
    adata[mask], color=label, ncols=2, frameon=True, return_fig=True, show=False
)
plot.set_size_inches(6, 6)
plot.savefig(
...
plot = sc.pl.umap(
    adata[mask], color="sample_id", ncols=4, frameon=True, return_fig=True, show=False
)
plot.set_size_inches(6, 6)
plot.savefig(
...
for ax, sub, title in [
    ...
    ax.grid(
        axis="y", which="major", color="black", linestyle="-", linewidth=1.2, zorder=3
    )
